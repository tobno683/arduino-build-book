...
{ id:'lis3dh',     name:'LIS3DH 3-axis accelerometer breakout', cat:'Sensor', price:5.00, lo:3, hi:12, unit:'each', q:'LIS3DH accelerometer breakout', buy:['adafruit','ali','amazon'],
  local:{se:{electrokit:'LIS3DH'}},
  note:'Chosen over an MPU6050 for three reasons: a hardware FIFO, a programmable activity interrupt that can wake a sleeping microcontroller, and 2 uA in low-power mode. For a battery sensor that mostly waits, those matter far more than resolution.' },
{ id:'rs485',      name:'MAX485 RS-485 transceiver module',      cat:'Module', price:1.20, lo:0.5, hi:4, unit:'each', q:'MAX485 RS485 module TTL', buy:['ali','amazon'],
  local:{se:{electrokit:'RS485'}},
  note:'Differential signalling over a twisted pair. Good for a kilometre where I2C manages about half a metre - which is the whole reason a long sensor string uses it.' },
{ id:'potting',    name:'Two-part epoxy potting compound, 500 g', cat:'Misc',   price:16.00, lo:9, hi:40, unit:'each', q:'two part epoxy potting compound electronics marine', buy:['amazon','ali'],
  note:'Fully encapsulates a node so there is no air gap for water to occupy. An IP68 box keeps water out until the seal fails once; potting has nothing to fail.' },
{ id:'cable-4c',   name:'Marine 4-core cable, 50 m',             cat:'Misc',   price:34.00, lo:20, hi:90, unit:'each', q:'4 core marine tinned copper cable 0.75mm', buy:['amazon','ali'],
  note:'Tinned copper, oil-and-water resistant sheath. Two cores for power and a twisted pair for data. Untinned copper in salt water goes green and open-circuit in a season.' },
...
{ id:'i2s-dac',    name:'I2S DAC board (PCM5102A)',              cat:'Module', price:6.00, lo:3, hi:14, unit:'each', q:'PCM5102A I2S DAC module', buy:['ali','amazon','adafruit'],
  local:{se:{electrokit:'PCM5102'}},
  note:'Sixteen-bit stereo with a real analogue output stage. The ESP32 has a built-in 8-bit DAC and it sounds like it - this is the single biggest quality difference available for the money.' },
...
{ id:'piezo-disc',  name:'Piezo disc, 27 mm brass-backed',        cat:'Passive', price:0.60, lo:0.2, hi:2, unit:'each', q:'27mm piezo disc element brass', buy:['ali','amazon'],
  local:{se:{electrokit:'piezoelement'}},
  note:'Scored into quadrants this becomes an X-Y-Z scanner with sub-nanometre resolution. The cheapest precision positioner in existence, and it is the same part as a buzzer.' },
{ id:'ptir-wire',   name:'Platinum-iridium wire, 0.25 mm, 90/10',  cat:'Misc',    price:22.00, lo:12, hi:60, unit:'10 cm', q:'platinum iridium wire 0.25mm 90/10 STM tip', buy:['amazon','ali'],
  note:'The tip. Pt/Ir does not oxidise, so a tip cut in air still works an hour later - tungsten needs etching and a vacuum to stay clean. Ten centimetres is hundreds of tips.' },
{ id:'electrometer', name:'Electrometer op-amp (LMP7721 or OPA129)', cat:'Module', price:9.00, lo:5, hi:24, unit:'each', q:'LMP7721 electrometer op amp low input bias', buy:['digikey','mouser'],
  note:'3 femtoamps of input bias current. An ordinary op-amp leaks more than the entire signal you are trying to measure - this single part is the difference between an image and noise.' },
{ id:'res100m',     name:'100 megohm resistor, 1%',               cat:'Passive', price:3.00, lo:1.5, hi:9, unit:'each', q:'100M ohm resistor 1% metal glaze', buy:['digikey','mouser','ali'],
  note:'The transimpedance feedback resistor - it turns 1 nA into 0.1 V. Handle it by the body only; a fingerprint across it is a lower resistance than the resistor.' },
{ id:'hopg',        name:'HOPG graphite sample, 10 x 10 mm',       cat:'Misc',    price:38.00, lo:20, hi:120, unit:'each', q:'HOPG highly oriented pyrolytic graphite SPM sample', buy:['amazon','ali'],
  note:'Peel the top layer off with sticky tape and you have an atomically flat surface, every time, in air. This is why every first STM images graphite rather than anything else.' },
{ id:'hv-piezo',    name:'High-voltage piezo driver board (+/-72 V)', cat:'Module', price:26.00, lo:15, hi:80, unit:'each', q:'piezo driver amplifier board high voltage +-72V', buy:['ali','amazon'],
  note:'A piezo needs tens of volts to move tens of nanometres. See the safety notes - it is low current, but it is not low voltage.' },
{ id:'viton',       name:'Viton O-rings, 20 mm, pack of 20',       cat:'Misc',    price:9.00, lo:4, hi:20, unit:'pack', q:'viton o-ring 20mm damping', buy:['amazon','ali'],
  note:'Stacked between steel plates these make the vibration isolator. Viton rather than nitrile: it damps instead of bouncing, which is the whole point.' },
...
{ id:'cd4051',     name:'CD4051 8-channel analogue multiplexer', cat:'Module', price:0.60, lo:0.3, hi:3, unit:'each', q:'CD4051 analog multiplexer DIP16', buy:['ali','amazon'],
  local:{se:{electrokit:'4051'}},
  note:'Three address pins select one of eight analogue inputs onto a single pin. Unlike a digital expander it passes real analogue voltages, which is what reading potentiometers needs.' },
...
/* --- enclosure / misc --------------------------------------------------- */
{ id:'uno-media',  name:'Arduino UNO Media Carrier',             cat:'Module', price:19.25, lo:19, hi:30, unit:'each', q:'Arduino UNO Media Carrier', buy:['arduino','digikey'],
  note:'Two 4-lane CSI camera connectors, a DSI display port and three audio jacks for the UNO Q and VENTUNO Q, with the other pins passed through. See the news shelf.' },
{ id:'smart-plug', name:'Certified smart plug with local API (Shelly or Tasmota)', cat:'Power', price:18.00, lo:12, hi:30, unit:'each', q:'Shelly plug local API', buy:['amazon'],
  local:{se:{kjell:'smart plugg'}},
  note:'Buy one that is certified for your country and can be switched over the LAN without a cloud service. It does the mains switching so that nothing you built does - which is the right division for anything safety-related.' },
{ id:'pla',        name:'PLA filament, 1 kg, 1.75 mm',           cat:'Print', price:18.00, lo:13, hi:32, unit:'1 kg', q:'PLA filament 1.75mm 1kg', buy:['amazon','ali'],
  local:{se:{electrokit:'PLA filament'}},
  note:'Easy to print and fine for most enclosures - but it softens at about 55 C. That rules it out near anything that gets warm, and a car dashboard in summer gets far hotter than that.' },
{ id:'petg',       name:'PETG filament, 1 kg, 1.75 mm',          cat:'Print', price:20.00, lo:15, hi:34, unit:'1 kg', q:'PETG filament 1.75mm 1kg', buy:['amazon','ali'],
  local:{se:{electrokit:'PETG filament'}},
  note:'The right default for electronics enclosures. Holds its shape to about 80 C, tough rather than brittle, and prints nearly as easily as PLA.' },
{ id:'asa',        name:'ASA filament, 1 kg, 1.75 mm',           cat:'Print', price:24.00, lo:18, hi:40, unit:'1 kg', q:'ASA filament 1.75mm 1kg UV resistant', buy:['amazon','ali'],
  note:'For outdoors and for heat - UV stable and good to about 95 C. Needs an enclosed printer, and it releases styrene while printing, so ventilate.' },
{ id:'tpu',        name:'TPU flexible filament, 95A, 500 g',     cat:'Print', price:19.00, lo:13, hi:32, unit:'500 g', q:'TPU 95A flexible filament 1.75mm', buy:['amazon','ali'],
  note:'Rubbery. Gaskets, bumpers, cable strain reliefs and feet. Print it slowly - it buckles in the extruder if pushed.' },
{ id:'heatset',    name:'Heat-set brass inserts, M3, 100 pcs',   cat:'Print', price:8.00,  lo:4, hi:16, unit:'100 pcs', q:'M3 heat set threaded insert brass', buy:['amazon','ali'],
  local:{se:{electrokit:'gängbussning'}},
  note:'Pressed into a printed hole with a soldering iron, they give a real metal thread that survives being opened hundreds of times. A screw straight into plastic survives about ten.' },
{ id:'calipers',   name:'Digital calipers, 150 mm',              cat:'Tool',  price:14.00, lo:8, hi:40, unit:'each', q:'digital vernier caliper 150mm', buy:['amazon','ali'],
  local:{se:{electrokit:'skjutmått',kjell:'skjutmått'}},
  note:'The single most important tool for designing around a real board. Measure your own board rather than trusting a drawing - clones differ by a millimetre, and a millimetre is the difference between fitting and not.' },
{ id:'fan60-pwm',  name:'60 mm 5 V PWM fan, 4-pin',              cat:'Motion', price:7.00, lo:4, hi:15, unit:'each', q:'60mm 5V PWM fan 4 pin', buy:['amazon','ali'],
  note:'Four wires: power, ground, PWM speed control and a tachometer. The tach wire is what lets you detect a dead fan - which matters, because a dead fan in a sealed case is how boards cook.' },
{ id:'ptc-heater', name:'PTC heating element, 12 V 60 C',        cat:'Power', price:6.00, lo:3, hi:12, unit:'each', q:'PTC heater 12V 60 degree ceramic', buy:['amazon','ali'],
  note:'Self-regulating: resistance climbs steeply at its rated temperature, so it cannot run away. That property is why it is the safe choice inside a plastic box.' },
